...
function RetailerProfilePage(): React.ReactElement {
  ...
  const fetchRetailerDetails = async () => {
    try {
      ...
      if (response.data && response.data.data) {
        ...
        setError("No retailer data returned from server.");
      }
    } catch (error: any) {
      if (error.response && error.response.data && error.response.data.detail) {
        setError(error.response.data.detail || "Failed to fetch retailer details.");
      } else {
        setError("An error occurred while fetching retailer details.");
      }
    }
  };
  useEffect(() => {
  ...
  async function handleSave() {
    ...
    try {
      ...
      fetchRetailerDetails();// Fetch the updated retailer data
      setRetailer(response.data.data);// Update the retailer data
      setProfilePicUrl(response.data.data.profile_picture);
      setIsEditing(false);
      alert('Retailer details saved successfully!');
    } catch (error: any) {
      console.error('There was an error updating the retailer data!', error);
      if (error.response && error.response.data && error.response.data.detail) {
        setError(error.response.data.detail || "Failed to save retailer details.");
      } else {
        setError("An error occurred while saving retailer details.");
      }
      alert('Failed to save retailer details.');
    }
  }
  ...
  return (
    <div className="flex min-h-screen bg-gray-50 dark:bg-slate-900">
      ...
      <div className="flex-1 ml-72 flex flex-col">
        <div className="sticky top-0 z-30 bg-gray-900 border-b border-slate-700 px-6 py-3 flex justify-end">
          <UserHeader />
        </div>
              ...
              {retailer ? (
                        ...
                        <div className="p-4 bg-gray-50 dark:bg-slate-700/30 rounded-lg">
                          ...
                          <div className="space-y-3">
                            <p className="flex justify-between">
                              ...
                              <span className="font-medium text-gray-900 dark:text-white">{retailer.first_name} {retailer.last_name}</span>
                            </p>
                            <p className="flex justify-between">
                              <span className="text-gray-600 dark:text-gray-400">Email:</span>
                              <span className="font-medium text-gray-900 dark:text-white">{retailer.email}</span>
                            </p>
                            <p className="flex justify-between">
                              <span className="text-gray-600 dark:text-gray-400">Phone:</span>
                              <span className="font-medium text-gray-900 dark:text-white">{retailer.phone}</span>
                            </p>
                            {retailer.phone2 && (
                              <p className="flex justify-between">
                                <span className="text-gray-600 dark:text-gray-400">Alternate Phone:</span>
                                <span className="font-medium text-gray-900 dark:text-white">{retailer.phone2}</span>
                              </p>
                            )}
                          </div>
                        </div>
                        ...
                        <div className="p-4 bg-gray-50 dark:bg-slate-700/30 rounded-lg">
                          ...
                          <div className="space-y-3">
                            <p className="flex justify-between">
                              ...
                              <span className="font-medium text-gray-900 dark:text-white">{retailer.address}</span>
                            </p>
                            <p className="flex justify-between">
                              <span className="text-gray-600 dark:text-gray-400">City:</span>
                              <span className="font-medium text-gray-900 dark:text-white">{retailer.city}</span>
                            </p>
                            <p className="flex justify-between">
                              <span className="text-gray-600 dark:text-gray-400">State:</span>
                              <span className="font-medium text-gray-900 dark:text-white">{retailer.state}</span>
                            </p>
                            <p className="flex justify-between">
                              <span className="text-gray-600 dark:text-gray-400">Country:</span>
                              <span className="font-medium text-gray-900 dark:text-white">{retailer.country}</span>
                            </p>
                            <p className="flex justify-between">
                              <span className="text-gray-600 dark:text-gray-400">Pin Code:</span>
                              <span className="font-medium text-gray-900 dark:text-white">{retailer.pin_code}</span>
                            </p>
                          </div>
                        </div>
                      </div>
                ...
                <div className="p-8 flex justify-center items-center">
                  <div className="flex items-center space-x-3">
                    ...
                    <span className="text-gray-600 dark:text-gray-300">Loading retailer details...</span>
                  </div>
                </div>
              )}
            </div>
