...
const Shipped: React.FC = () => {
  ...
  const handleVisibilityChange = () => {
    if (document.visibilityState === 'visible') {
      const shouldRefresh = sessionStorage.getItem('forceRefreshShipped') === 'true';
      if (shouldRefresh) {
        console.log('Force refreshing Shipped orders due to navigation from Processing page');
        sessionStorage.removeItem('forceRefreshShipped');
        fetchBookings();
      }
    }
  };
  ...
  const fetchBookings = async () => {
    try {
      ...
      const shippedBookings = processedBookings.filter(booking => booking.status === 'shipped');
      setBookings(shippedBookings);
      setError(null);
    } catch (err) {
  ...
  const updateBookingStatus = async (bookingId: string, status: string) => {
    try {
      ...
      if (status === 'delivered') {
        ...
        setUpdateSuccess(`Booking ${bookingId} successfully marked as delivered and moved to the Completed Orders page.`);
        
        sessionStorage.setItem('forceRefreshComplete', 'true');
      } 
      else {
        setBookings(prevBookings => 
  ...
  useEffect(() => {
    ...
    const shouldRefresh = sessionStorage.getItem('forceRefreshShipped') === 'true';
    if (shouldRefresh) {
      console.log('Initial refresh of Shipped orders due to navigation from Processing page');
      sessionStorage.removeItem('forceRefreshShipped');
    }
        ...
        <main className="flex-1 p-6">
          ...
          <div className="flex items-center justify-between mb-6">
            <h1 className="text-2xl font-bold text-slate-800 dark:text-white flex items-center">
              <FiPackage className="mr-2 text-purple-600 dark:text-purple-400" />
              Shipped Orders
            </h1>
            ...
            <div className="flex space-x-3">
              <div className="hidden sm:flex items-center text-sm text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-slate-700 px-3 py-1 rounded-md">
                ...
                Mark as "delivered" to move to Completed Orders
              </div>
              <Button 
                variant="contained" 
