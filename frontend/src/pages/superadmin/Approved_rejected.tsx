...
const ApprovedRejected: React.FC = () => {
    ...
    <ErrorBoundary>
      <div className="flex min-h-screen bg-gray-900 text-gray-100 overflow-hidden">
        ...
        <div className="flex-1 flex flex-col w-full ml-0 lg:ml-72">
          ...
          <main className="flex-1 p-6 bg-gray-900 text-gray-100 overflow-y-auto">
            {error && (
              <div className="bg-red-900/20 border-l-4 border-red-500 text-red-400 p-4 mb-6 rounded-md">
            ...
            <div className="flex flex-wrap items-center justify-between mb-6">
              ...
              <div className="flex space-x-3">
                <select
                  className="bg-gray-800 border-gray-700 text-gray-200 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  value={filterStatus}
            ...
            {loading ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {currentItems.length === 0 ? (
                  <div className="col-span-full flex flex-col items-center justify-center h-60 bg-gray-800 rounded-xl border border-gray-700">
                  ...
                  currentItems.map((item, index) => (
                    <div
                      ...
                      className={`bg-gray-800 rounded-xl border border-gray-700 hover:shadow-md transition-all duration-300 overflow-hidden ${
                    ...
                    >
                      <div className={`p-4 ${
                        getStatus(item) === "approved" 
                          ? 'bg-gradient-to-r from-green-900/30 to-green-900/20' 
                          : 'bg-gradient-to-r from-red-900/30 to-red-900/20'
                      }`}>
                        <div className="flex justify-between items-center">
                          <h2 className="flex items-center font-bold text-gray-100">
                            <svg xmlns="http://www.w3.org/2000/svg" className={`h-5 w-5 mr-1.5 ${
                              getStatus(item) === "approved" ? 'text-green-500' : 'text-red-500'
                      ...
                      <div className="p-5">
                        ...
                        <div className="space-y-3">
                          <div className="flex items-center justify-between">
                            <span className="text-sm font-medium text-gray-400">Furniture Title:</span>
                            <span className="text-sm font-bold text-gray-100">{getTitle(item)}</span>
                          </div>
                          
                          <div className="flex items-center justify-between">
                            <span className="text-sm font-medium text-gray-400">Creator:</span>
                          ...
                          <div className="flex items-center justify-between">
                            <span className="text-sm font-medium text-gray-400">{getPriceLabel(item)}</span>
                            <span className="text-sm font-bold text-gray-100">
                              ${getPrice(item)}
                              {isItemForRent(item) && <span className="text-xs text-gray-400 ml-1">/day</span>}
                            </span>
                          </div>
                          
                          <div className="flex items-center justify-between">
                            <span className="text-sm font-medium text-gray-400">Category:</span>
                            <span className="text-sm font-bold text-gray-100">{getCategory(item)}</span>
                          </div>
                          ...
                          {getReviewNote(item) && (
                            <div className="mt-3 p-3 bg-gray-700/50 rounded-lg">
                              <p className="text-sm font-medium text-gray-400 mb-1">Review Note:</p>
                              <p className="text-sm text-gray-100">{getReviewNote(item)}</p>
                            </div>
                          )}
                        </div>
                      </div>
              ...
              <div className="flex justify-center mt-8">
                <nav className="flex items-center gap-2">
                  <button
                    ...
                    className="px-3 py-1 rounded bg-gray-800 text-gray-300 disabled:opacity-50 hover:bg-gray-700 border border-gray-700"
                    title="First Page"
                  >
                    &laquo;
                  </button>
