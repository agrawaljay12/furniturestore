...
interface Product {
  ...
  discountedPrice: string; // Make discountedPrice required
  discount?: number; // Add the missing property
}
...
const ChairsPage: React.FC = () => {
  ...
  const [sortBy, setSortBy] = useState("title"); // Changed from constant to state
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      const bodyContent = JSON.stringify({
        ...
        page_size: 100,
        sort_by: sortBy,
        sort_order: sortOrder, // Added sort order
        search: searchQuery,
        title: "chairs" // Filter by chairs category
      });
  ...
  const handleAddToCart = (product: Product, quantity: number) => {
    ...
    // Ensure product has a discountedPrice and discount is a number before adding to cart
    const productWithDiscountedPrice = {
      ...product,
      discountedPrice: product.discountedPrice || product.rent_price,
      discount: product.discount ? Number(product.discount) : 0 // Convert to number or default to 0
    };
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-gray-50 px-8 pt-24 pb-16">
        <h2 className="text-3xl font-semibold text-center mb-8">Chairs Furniture</h2>

        {/* Add sort and filter section */}
        <div className="flex justify-between items-center mt-12 px-6">
          <div>
            <h2 className="text-4xl font-sans text-gray-800 section-title">Chair Collection</h2>
            <p className="text-lg text-gray-600 mt-2">Comfort and style for your space</p>
          </div>
              ...
              <div className="p-4 flex-grow flex flex-col justify-between">
                ...
                <div className="mt-4">
                  <div className="flex justify-between items-center mb-3">
                    ...
                    className="w-full py-2 px-4 bg-yellow-400 text-black font-medium rounded-lg hover:bg-yellow-500 transition-all transform hover:scale-105 flex items-center justify-center"
                    >
          ...
          >
            ...
            <div className="mb-4">
              <div className="flex justify-between items-center mb-2">
                <div className="text-2xl font-bold text-gray-900">${(parseFloat(previewProduct.rent_price) * quantity).toFixed(2)}<span className="text-sm text-gray-500">/day</span></div>
