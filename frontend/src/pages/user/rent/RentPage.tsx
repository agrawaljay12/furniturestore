...
interface Product {
  ...
  discountedPrice?: string; // Changed to string to match CartContext
...
const RentPage: React.FC = () => {
    ...
    const fetchProducts = async () => {
      ...
      try {
        ...
        if (data && data.data) {
          ...
          setProducts(filteredProducts); // Only approved products for rent
        }
      } catch (error) {
  ...
  const handleAddToCart = (product: Product, quantity: number) => {
    ...
    const productWithDiscountedPrice = {
      ...product,
      discountedPrice: product.discountedPrice || product.price, // Use regular price if no discount
      ...
      <div className="min-h-screen bg-gray-50 px-8 pt-24 pb-16">
        ...
        <div className="p-10 bg-transparent">
          <div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-8 justify-items-center ">
            {[
              { label: "Living Room", icon: couchImage, badge: "", badgeColor: "bg-yellow-400", link: "/rent-living" },
              { label: "Bedroom", icon: bedimage, badge: "", badgeColor: "bg-yellow-400", link: "/rent-bedroom" },
              { label: "Storage", icon: storeimage, badge: "", badgeColor: "", link: "/rent-storage" },
              { label: "Dining", icon: diningimage, badge: "", badgeColor: "", link: "/rent-dining" },
              { label: "Tables", icon: deskimage, badge: "", badgeColor: "", link: "/rent-tables" },
            ].map((item) => (
              <div
                key={item.label}
          ...
          <div className="flex justify-center gap-28 mt-20">
            {[
              { label: "Chairs", icon: chairimage, badge: "", badgeColor: "", link: "/rent-chairs" },
              { label: "Best Deals", icon: dealimage, badge: "", badgeColor: "", link: "/rent-best-deals" },
              { label: "Mattress", icon: mattress, badge: "", badgeColor: "", link: "/rent-mattress" },
            ].map((item) => (
              <div
                key={item.label}
                className="relative flex flex-col items-center transition-transform duration-300 hover:scale-110 hover:text-blue-500 cursor-pointer"
        ...
        {/* Remove the existing sorting section and replace with the new one */}
        {/* Sort and Filter Section */}
        <div className="flex justify-between items-center mt-12 px-6">
          <div>
            <h2 className="text-4xl font-sans text-gray-800 section-title">Furniture for Rent</h2>
            <p className="text-lg text-gray-600 mt-2">Our latest rental collection</p>
          </div>
              ...
              <div className="p-4 flex-grow flex flex-col justify-between">
                ...
                <div className="mt-4">
                  <div className="flex justify-between items-center mb-3">
                    ...
                    className="w-full py-2 px-4 bg-yellow-400 text-black font-medium rounded-lg hover:bg-yellow-500 transition-all transform hover:scale-105 flex items-center justify-center gap-2"
                    >
        ...
        {previewProduct && (
          <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50 backdrop-blur-sm" 
               ...
               >
                 ...
                 <button 
                   onClick={closePreview} 
                   className="text-gray-500 hover:text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-full p-2 transition-colors text-xl font-bold leading-none flex items-center justify-center w-8 h-8"
                 >
                   ×
                 </button>
                 </div>
