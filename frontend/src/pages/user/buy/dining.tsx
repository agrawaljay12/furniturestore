...
interface Product {
  ...
  discountedPrice?: string; // Added for CartContext compatibility
  discount?: number; // Added for CartContext compatibility
}
...
const DiningPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      const bodyContent = JSON.stringify({
        ...
        page_size: 100,
        sort_by: sortBy,
        sort_order: sortOrder, // Add sort order to request
        search: searchQuery,
        title: "Dining", // Filter by dining category
        is_for_sale: true // Only fetch items that are for sale
      });
    ...
    fetchProducts();
  }, [searchQuery, sortBy, sortOrder, page]); // Add sortOrder dependency
  ...
  const handleAddToCart = (product: Product, quantity: number) => {
    ...
    // Ensure discountedPrice is a string and discount is a number
    const productWithStringDiscountedPrice = {
      ...product,
      discountedPrice: product.discountedPrice || product.price, // Default to regular price if no discount
      discount: product.discount || 0 // Ensure discount is always a number, default to 0
    };

    addToCart(productWithStringDiscountedPrice, quantity);
    alert("Added to cart!");
    closePreview();
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-gray-50 px-8 pt-24 pb-16">
        <h2 className="text-3xl font-semibold text-center mb-8">Dining Furniture</h2>

        {/* Sort and Filter Section */}
        <div className="flex justify-between items-center mt-12 px-6">
          <div>
            <h2 className="text-4xl font-sans text-gray-800 section-title">Dining Collection</h2>
            <p className="text-lg text-gray-600 mt-2">Find the perfect dining set</p>
          </div>
                      ...
                      >
                        ...
                        {sortBy === option.id && (
                          <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-yellow-600" viewBox="0 0 20 20" fill="currentColor">
                            <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 011.414-1.414L8 12.586l7.293-7.293a1 1 011.414 0z" clipRule="evenodd" />
                          </svg>
                        )}
                      </button>
                    </div>
                ...
                <div className="mt-4 space-y-3">
                  ...
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                      <path d="M3 1a1 1 0 000 2h1.22l.305 1.222a.997.997 0 00.01.042l1.358 5.43-.893.892C3.74 11.846 4.632 14 6.414 14H15a1 1 000-2H6.414l1-1H14a1 1 00.894-.553l3-6A1 1 000-1H6.28l-.31-1.243A1 1 000-1H3z" />
                    </svg>
              ...
              <div className="text-center mb-6">
                <div className="inline-block p-3 bg-blue-100 rounded-full mb-4">
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 text-blue-600" viewBox="0 0 20 20" fill="currentColor">
                    <path fillRule="evenodd" d="M18 10a8 8 0116 0 8 8 0116 0zm-7-4a1 1 011-2 0 1 1 012 0zM9 9a1 1 000 2v3a1 1 001 1h1a1 1 100-2v-3a1 1 00-1-1H9z" clipRule="evenodd" />
                  </svg>
                </div>
