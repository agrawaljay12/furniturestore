...
const DealsPage: React.FC = () => {
    ...
    const fetchDiscountedProducts = async () => {
      ...
      try {
        ...
        if (data && Array.isArray(data.data)) {
          ...
          // Filter products to only include those that are for sale
          let filteredProducts = showSaleOnly ? 
            processedProducts.filter((product: Product) => product.is_for_sale) : 
            processedProducts;
          
          console.log(`${filteredProducts.length} products are available for sale out of ${processedProducts.length} total`);
          
          // If no products are available for sale, show a message
          if (filteredProducts.length === 0) {
            setError("No furniture items are currently available for sale in deals");
            setProducts([]);
            setLoading(false);
            return;
          }
  ...
  }, [searchQuery, sortBy, sortOrder, page, showSaleOnly]); // Add showSaleOnly dependency
  ...
  const handleAddToWishlist = (product: Product, e: React.MouseEvent) => {
    ...
    if (isInWishlist) {
      ...
      // Create a new object to avoid reference issues with the same product ID
      const productToAdd = { ...product, wishlistId: `${product._id}-${Date.now()}` };
      updatedWishlist = [...wishlist, productToAdd];
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-gray-50 px-8 pt-24 pb-16">
        ...
        {/* Filter options - Just like rent deals page */}
                      ...
                      >
                        ...
                        {sortBy === option.id && (
                          <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-yellow-600" viewBox="0 0 20 20" fill="currentColor">
                            <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 01-1.414 0l-4-4a1 1 01 1.414-1.414L8 12.586l7.293-7.293a1 1 011.414 0z" clipRule="evenodd" />
                          </svg>
                        )}
                      </button>
                    </div>
        ...
        {/* Add an informative banner about sale items - Yellow version for buy pages */}
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6 mt-8">
            {displayedProducts.map((product, index) => (
              <div
                key={`${product._id}-${index}`} // Add index to ensure uniqueness
                    ...
                    <div className="flex justify-between items-center">
                      {product.discount ? (
                        <div className="flex flex-col">
                          <div className="text-xl font-bold text-green-600">${getDiscountedSalePrice(product)}</div>
          ...
          <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50 backdrop-blur-sm" 
            ...
            >
              <div className="flex justify-between items-center mb-4">
                ...
                >
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                    <path fillRule="evenodd" d="M4.293 4.293a1 1 011.414 0L10 8.586l4.293-4.293a1 1 011.414 1.414L11.414 10l4.293 4.293a1 1 01-1.414 1.414L8.586 10 4.293 5.707a1 1 010-1.414z" clipRule="evenodd" />
                  </svg>
                </button>
              </div>
              ...
              <div className="relative rounded-lg overflow-hidden mb-4">
                ...
                {previewProduct.discount && (
                  <div className="absolute bottom-4 right-4 bg-red-500 text-white px-3 py-1 rounded-lg text-sm font-bold animate-pulse">
                  ...
                  {previewProduct.discount ? (
                    <div className="flex flex-col">
                      <div className="text-2xl font-bold text-green-600">
                        ${(parseFloat(getDiscountedSalePrice(previewProduct)) * quantity).toFixed(2)}
                      </div>
              ...
              <div className="flex space-x-3">
                <button
                  ...
                  className="flex-1 py-3 bg-yellow-500 text-gray-900 rounded-lg hover:bg-yellow-400 transition-all flex items-center justify-center space-x-2"
                >
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                    <path d="M3 1a1 1 0 0 0 0 2h1.22l.305 1.222a.997.997 0 0 0 .01.042l1.358 5.43-.893.892C3.74 11.846 4.632 14 6.414 14H15a1 1 0 0 0 0-2H6.414l1-1H14a1 1 0 0 0 .894-.553l3-6A1 1 0 0 0 17 3H6.28l-.31-1.243A1 1 0 0 0 5 1H3z" />
                  </svg>
                  <span>
                    {previewProduct.discount ? 
                      `Buy for $${getDiscountedSalePrice(previewProduct)}` : 
                      `Add to Cart ($${previewProduct.price})`}
                  </span>
                </button>
                <button
                  onClick={(e) => {
                    handleAddToWishlist(previewProduct, e);
                    closePreview();
                  }}
